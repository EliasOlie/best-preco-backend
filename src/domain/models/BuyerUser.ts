...
class BuyerUser extends BaseUser{
    ...
    //CPF Wont change
    private isCpfValid(cpf: any){ //https://gist.github.com/joaohcrangel/8bd48bcc40b9db63bef7201143303937?permalink_comment_id=3781326#gistcomment-3781326
    if (typeof cpf !== 'string') return false
    cpf = cpf.replace(/[^\d]+/g, '')
    if (cpf.length !== 11 || !!cpf.match(/(\d)\1{10}/)) return false
    cpf = cpf.split('')
    const validator = cpf
        .filter((digit: number, index:number, array: Array<number>) => index >= array.length - 2 && digit)
        .map( (el: number) => +el )
    const toValidate = (pop: number) => cpf
        .filter((digit: number, index:number, array: Array<number>) => index < array.length - pop && digit)
        .map((el: number) => +el)
